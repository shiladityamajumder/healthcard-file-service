...
All mappings in this service were verified against `healthcare_db`. The Python project remains the only migration authority. This service sets TypeORM `synchronize: false` and `migrationsRun: false`, contains no migration files, and never creates schemas or tables.
